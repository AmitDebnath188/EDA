...
try:
    ...
    from neurokit2 import signal_filter, signal_findpeaks, signal_smooth, signal_zerocrossings



def eda_findpeaks(eda_phasic, sampling_rate=128, method="gamboa2008", amplitude_min=0.1):
    ...
    * Nabian, M., Yin, Y., Wormwood, J., Quigley, K. S., Barrett, L. F., & Ostadabbas, S. (2018). An
    ...
    if isinstance(eda_phasic, (pd.DataFrame, pd.Series)):
        if "EDA_Phasic" in eda_phasic.columns if isinstance(eda_phasic, pd.DataFrame) else False:
            eda_phasic = eda_phasic["EDA_Phasic"].values
        else:
            eda_phasic = np.array(eda_phasic).flatten()
...
def _eda_findpeaks_vanhalem2020(eda_phasic, sampling_rate=128):
...
def _eda_findpeaks_kim2004(eda_phasic, sampling_rate=128, amplitude_min=0.1):
...
def _eda_findpeaks_nabian2018(eda_phasic):
    ...
    * Nabian, M., Yin, Y., Wormwood, J., Quigley, K. S., Barrett, L. F., & Ostadabbas, S. (2018). An
    ...
    # }
    info = {
        "SCR_Onsets": np.array(info["SCR_Onsets"]),
        "SCR_Peaks": np.array(info["SCR_Peaks"]),
        "SCR_Height": np.array(info["SCR_Height"]),
    }
